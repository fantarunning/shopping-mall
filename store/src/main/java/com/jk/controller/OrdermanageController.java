...
public class OrdermanageController {
    ...
    @GetMapping("/list")
    public List<Ordermanage> list(){
        return ordermanageService.list();
    }
    ...
    public ResultUtil list( Integer page,Integer limit,
                            String orderNum){
        System.out.println(orderNum+","+page+","+limit);
//        传入分页属性
        Page<Ordermanage> objectPage = new Page<>(page, limit);
//        查询条件
        QueryWrapper<Ordermanage> wrapper = new QueryWrapper<>();
        wrapper.like(StringUtils.isNotBlank(orderNum),"order_num",orderNum).orderByDesc("id");
//        分页查询
        Page<Ordermanage> Page = ordermanageService.page(objectPage, wrapper);
//        获取数据总条数
        long total = Page.getTotal();
//        数据
        List<Ordermanage> ordersList = Page.getRecords();

        return new ResultUtil(200,total,ordersList);
    }

    //    提交数据-->新增、修改
    @PostMapping("/save")
    public ResultUtil add(@RequestBody Ordermanage Ordermanage){
        if (Ordermanage.getId()==null){//新增
            try {
                Date date = new Date();
                Ordermanage.setCreateTime(date);
    ...
    public String del(int id){
        boolean b = ordermanageService.removeById(id);
        if (b){
            return "1";
        }else {
            return "0";
    ...
    public ResultUtil delBatch(@RequestBody List<Integer> ids){
        ...
        ordermanageService.removeByIds(ids);
    ...
    public void export(HttpServletResponse response) throws Exception {
        // 从数据库查询出所有的数据
        List<Ordermanage> list = ordermanageService.list();
...
//        ExcelWriter writer = ExcelUtil.getWriter(filesUploadPath + "/用户信息.xlsx");
    ...
    public ResultUtil imp(MultipartFile file) throws Exception {
        ...
        List<Ordermanage> list = reader.readAll(Ordermanage.class);
        ...
        ordermanageService.saveBatch(list);
